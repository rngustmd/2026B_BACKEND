...
public class BoardDao extends BaseDao {
    ...
    public ArrayList<BoardDto> findAll( ){
        ...
        // 2.3 ?매개변수 대입한다. <생략>
        // 2.4 기재된 SQL 실행 , .executeQuery(); select
        ResultSet rs = ps.executeQuery();
        // 2.5 SQL 결과( select 조회 결과는 항상 테이블로 반환한다. ) 즉] 레코드 하나씩 타입변환
        while( rs.next() ){ // rs.next() : 다음 레코드(행) 이동 , 마지막 레코드까지 반복 뜻
    ...
    public boolean update( BoardDto boardDto ){
        try{
            ...
            PreparedStatement ps = conn.prepareStatement(sql); // 1.2 SQL 기재
            ps.setString(1, boardDto.getContent() ); // 1.3 SQL 내 ? 매개변수대입
    ...
    public boolean delete( int no ){
        try{ String sql = "delete from board where no = ?";
            PreparedStatement ps = conn.prepareStatement( sql );
            ps.setInt( 1, no ); // SQL 문법 내 첫번째 ? 에 매개변수 값 대입
