...
create table test3(
    ...
    필드명4 datetime default now(), -- 예] 레코드 삽입 시 현재날짜/시간 자동 대입
    필드명5 bigint auto_increment primary key, 
    constraint primary key( 필드명5 ) -- 특정 필드/속성을 pk로 설정한다.
    -- auto_increment: 해당 필드/속성에 레코드(행) 삽입 시 자동으로 순서번호 설정 , 1 2 3 4 5 6 ~
