...
SELECT min( bamunt ) from buy; -- min( 필드명 ) 최솟값
SELECT max( bamunt ) FROM buy; -- max( 필드명 ) 최댓값
...
SELECT MID , sum( bamount ) 총구매수량 FROM buy GROUP BY MID HAVING 총구매수량 > 5 ;   -- 총구매수량 5초과이면
# where 절 에서 필드의 별칭 사용이 안된다. 왜? where 먼저 처리
# SELECT MID , sum( bamount ) 총구매수량 FROM buy WHERE 총구매수량 > 5 GROUP BY MID ; -- 오류

# [5] order by 정렬 , desc내림차순( 3 2 1 , 다 나 가 , C B A , 0811 0810 0809 ) , asc오름차순(기본값)
SELECT * FROM member ORDER BY mdebut;
SELECT * FROM ORDER BY mdebut DESC;
# [*] 다중정렬이란? 첫번째 정렬 후 첫번째 정렬 필드 기준으로 중복이 존재한 경우 중복끼리 2차정렬
-- 1차정렬 지역(maddr) 먼저 정렬하고 만약에 지역 필드내 동일한 값 끼리 2차정렬( mdebut ) 한다.
...
SELECT * FROM member LIMIT 0 , 2; -- 1 ~ 2 ( 0번부터 2개 ) (2)
