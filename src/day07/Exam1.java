    ...
    public static void main(String[] args) {
        ...
        메소드
            ...
            3) 선언
                ...
                - 반환타입 : 반환값의 타입/자료형 일치!!
                - 메소드명 : 소문자 시작하는 카멜표기법
                - 타입 매개변수 : 함수 호출하는 곳으로부터 전달받는 값 타입 일치!
                - { } : 함수호출시 실행할 코드
                - return : 함수종료시 반환되는 값
            4) 호출 : 다양한 기준
                1. 서로 다른 클래스에서 메소드 호출
                    ...
                    객체변수먕.매소드명()
                2. 동일한 클래스 내 메소드 호출시 인스턴스 필요하다.
                    메소드명( );
            *) 매개변수는 지역변수 이다.
                지역변수란? 특정 함수/if/for{} 에서 선언된 변수는 종료되면 사라진다.
...
class 계산기{
    ...
    // 4) 매개변수 o , 반환값 o
    int add( int x , int y ){
        // 동일한 클래스 내 다른 메소드 호출 => 인스턴스 없어도 됨
        printSum(x, y); // this.~~ 도 가능
        return x + y ;
    }
}
