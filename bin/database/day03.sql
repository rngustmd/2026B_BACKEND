...
CREATE TABLE test( -- creat table 테이블명( 속성명 타입 제약조건 , 속상명 타입 제약조건 )
...
INSERT INTO test values( 4 , "하하" , 30 ); -- 모든 속성값 정의 순서대로 값 대입시 속성명 생략 가능
...
SELECT * FROM test WHERE 이름 = "유재석"; -- =같다 , 이름 속성명의 값이 유재석이면 조회
SELECT * FROM test WHERE 개수 >= 5; -- 개수 속성명의 값이 5 이상이면 조회

# [3] update : update 테이블명 set 속성명 = 새로운값 , 속성명 = 새로운값 where 조건
UPDATE test SET 개수 = 10; -- 조건이 없으므로 테이블내 '개수' 속성들의 값 *모두* 10으로 수정
UPDATE test SET 개수 = 30 where 이름 = "유재석"; -- 이름 속성값 유재석이면 개수 속성값을 30으로 수정
UPDATE test SET 개수 = 40 , 이름 = "강호동2" where 번호 = 2; -- 번호 속성값이 2 인 개수속성값을 40, 이름속성값을 강호동2로 수정

# [4] delete : delete from 테이블명 where 조건
-- delete from test; -- 조건이 없으믈 테이블내 모든 레코드 제거
DELETE FROM test WHERE 이름 = "유재석"; -- 이름 속성값이 2이면 삭제
DELETE FROM test WHERE 번호 = 2; -- 번호 속성값이 2 이면 삭제
-- delete : 테이블내 레코드 삭제 [vs] truncate table : 테이블내 레코드 삭제 [vs] drop table : 테이블 자체 삭제
-- DDL 은 취소 불가능 , DML(S/I/U/D) 취소(ROLLBACK)
...
SELECT * FROM member WHERE mheight <= 162; -- '키' 속성값이 162 이하라면
...
-- 데이터베이스 null 처리방법 , = null 대신에 is null , != null 대신에 is not null
SELECT * FROM member WHERE mphone1 = null; 
SELECT * FROM member WHERE mphone1 IS NULL; -- 주의할점 : =null[x] 대신에 is null[0]
