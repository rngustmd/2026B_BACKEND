...
SELECT customer , COUNT(*) AS 주문건수 FROM orders GROUP BY customer;
-- [문제 11 ] orders 테이블에서 도서번호(book_id)별 총 주문수량 합계를 조회하세요. (book_id, sum)
SELECT book_id, SUM(order_qty) AS 총주문수량 FROM orders GROUP BY book_id;
-- [문제 12 ] orders에서 고객별 총 주문수량 합계가 5 이상인 고객만 조회하세요. ( 출력: customer, 총주문수량, 조건: HAVING 사용 )
SELECT customer, SUM(order_qty) AS 총주문수량 FROM orders GROUP BY customer HAVING SUM(order_qty) >= 5;
-- [문제 13 ] orders에서 도서번호(book_id)별 주문 건수가 3회 이상인 도서만 조회하세요.( 출력: book_id, 주문건수 )
SELECT book_id , COUNT(*) AS 주문건수 FROM orders GROUP BY book_id HAVING COUNT(*) >= 3;
